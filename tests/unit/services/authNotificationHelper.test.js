...
describe('AuthNotificationHelper - Unit Tests', () => {
  ...
  beforeEach(() => {
    ...
    helper = require('../../../src/services/authNotificationHelper');
  ...
  it('_extractId should handle string and object with _id', () => {
    expect(helper._extractId('abc')).toBe('abc');
    expect(helper._extractId({ _id: { toString: () => 'id-1' } })).toBe('id-1');
  });
});
