...
const { riskRepository, caseBaseRepository, projectRepository, userRepository } = require('../../repositories');
const decisionTreeService = require('./decisionTree');
const cbrService = require('./cbr.service');
const teamAnalysisService = require('../team/teamAnalysis.service');
...
const CATALOG_TYPES = new Set(i18n.getRiskTypes());
...
function normalizeRiskFromCatalog(risk, lang = 'es') {
  const metadata = i18n.getRiskMetadata(risk?.type, lang);
...
async function predictProjectRisks(projectId, lang = 'es') {
  try {
    ...
    const caseBaseStats = await caseBaseRepository.getCaseBaseStats(organizationId);
    ...
    throw new AppError('RISK_PREDICTION_FAILED', 500, `Risk prediction failed: ${error.message}`);
  }
}
...
async function saveRiskPredictions(project, risks, metadata) {
  ...
  await riskRepository.deleteMany({ project: project._id, occurred: null });
  ...
  const savedRisks = await riskRepository.insertMany(riskDocs);
...
async function getProjectRiskPredictions(projectId, options = {}) {
  ...
  const risks = await riskRepository.getProjectRisks(projectId, options);
...
async function getOrganizationRiskInsights(organizationId) {
  const [riskStats, caseBaseStats] = await Promise.all([
    riskRepository.getOrganizationStats(organizationId),
    caseBaseRepository.getCaseBaseStats(organizationId)
  ]);
  
  const accuracyReport = await riskRepository.getAccuracyReport(organizationId);
...
function generateTeamInsights(team, organization) {
  ...
  // Validate that team exists and has valid structure
  if (!team) {
    return insights;
  }
