...
async function predictRisksWithCBR(project, organizationId, k = 5, teamAnalysis = null) {
  try {
    ...
    return {
      ...
      message: `Prediction based on ${similarCases.length} similar case(s)`
    };
  } catch (error) {
    console.error('Error in CBR prediction:', error);
    throw new AppError('CBR_PREDICTION_FAILED', 500, `CBR prediction failed: ${error.message}`);
  }
}
...
async function retrieveSimilarCases(project, organizationId, k = 5, teamAnalysis = null, minSimilarity = 0.3) {
  // Get all cases for the organization (real + seed) — lean avoids hydrating full Mongoose docs
  const realCases = await caseBaseRepository.find({
    organization: organizationId,
    type: 'real'
  }, { lean: true });
  
  const seedCases = await caseBaseRepository.find({
    type: 'seed'
  }, { lean: true });
  ...
  topCases.forEach(async (sc) => {
    if (!sc.isGeneric) {
      await caseBaseRepository.updateById(sc.case._id, { $inc: { 'metadata.timesReused': 1 }, $set: { 'metadata.lastReusedAt': new Date() } });
    }
  });
...
function reuseSolution(similarCases, project) {
  ...
  Object.entries(risksByType).forEach(([riskType, occurrences]) => {
    ...
    predictedRisks.push({
      ...
      reasoning: [
        `Based on project "${primary.projectName}" (similarity: ${(primary.similarity * 100).toFixed(0)}%)`,
        ...(otherCases.length > 0 ? [`Also occurred in ${otherCases.length} similar project(s)`] : [])
      ],
      indicators: [
        `Source: ${primary.originalSource === 'manual' ? 'Manually identified' : 'Predicted and confirmed'}`,
        `Found in ${occurrences.length} similar project(s)`
      ],
  ...
  treeRisks.forEach(treeRisk => {
    ...
    if (!existsInCBR) {
      revisedRisks.push({
        ...
        note: 'Detected by expert rules (not seen in similar cases)'
      });
    }
  });
...
async function retainCase(project, postProjectData, organization) {
  ...
  let existingCase = await caseBaseRepository.findOne({ caseId: project._id });
  
  if (existingCase) {
    // Update existing case instead of creating a new one
    existingCase = await caseBaseRepository.updateById(existingCase._id, {
      problem: {
      ...
      'metadata.completedAt': postProjectData.completedAt || new Date(),
      'metadata.tags': extractTags(project, postProjectData),
      updatedAt: new Date()
    });
  ...
  const newCase = await caseBaseRepository.create({
    caseId: project._id,
...
async function calculateCBRConfidence(similarCases, organizationId) {
  const totalCases = await caseBaseRepository.count({});
