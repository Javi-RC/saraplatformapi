...
const { projectRepository, caseBaseRepository, riskRepository, userRepository } = require('../../repositories');
...
async function captureProjectOutcome(projectId, outcomeData, userId) {
  try {
    ...
    if (!project.organization) {
      throw AppError.badRequest('PROJECT_NO_ORGANIZATION', 'Project must have an associated organization');
    }
    ...
    if (!canUpdateOutcome(project, userId)) {
      throw AppError.forbidden('NOT_AUTHORIZED', 'Not authorized to update project outcome');
    }
    ...
    const allProjectRisks = await riskRepository.find({
      project: projectId
    });
    ...
    throw AppError.badRequest('OUTCOME_CAPTURE_FAILED', `Failed to capture outcome: ${error.message}`);
  }
}
...
function validateOutcomeData(data) {
  if (data.completed === undefined) {
    throw AppError.badRequest('COMPLETED_FIELD_REQUIRED', 'completed field is required');
  }
  
  if (data.qualityScore && (data.qualityScore < 1 || data.qualityScore > 5)) {
    throw AppError.badRequest('INVALID_QUALITY_SCORE', 'qualityScore must be between 1 and 5');
  }
  
  if (data.clientSatisfaction && (data.clientSatisfaction < 1 || data.clientSatisfaction > 5)) {
    throw AppError.badRequest('INVALID_CLIENT_SATISFACTION', 'clientSatisfaction must be between 1 and 5');
  }
  
  if (data.teamMorale && (data.teamMorale < 1 || data.teamMorale > 5)) {
    throw AppError.badRequest('INVALID_TEAM_MORALE', 'teamMorale must be between 1 and 5');
  }
  
  if (data.budgetOverrun && data.budgetOverrun < 0) {
    throw AppError.badRequest('INVALID_BUDGET_OVERRUN', 'budgetOverrun cannot be negative');
  }
}
...
async function generateLearningReport(project, newCase) {
  const predictedRisks = await riskRepository.find({
    project: project._id
  });
  ...
  const similarPastCases = await caseBaseRepository.findSimilar(
  ...
  return {
    ...
    systemImpact: {
      caseBaseSize: (await caseBaseRepository.getCaseBaseStats(organizationId)).total,
...
async function getPostProjectForm(projectId, lang = 'es') {
  ...
  if (project.status !== 'completed') {
    throw AppError.badRequest('PROJECT_NOT_COMPLETED', `Project must be marked as completed before capturing outcome. Current status: ${project.status}`);
  }
