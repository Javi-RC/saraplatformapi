...
class BFI44Service {
  ...
  static calculateFactorScore(responses, factorName) {
    ...
    for (const itemId of items) {
      ...
      if (value === undefined) {
        throw AppError.badRequest('MISSING_ITEM', `Missing item ${itemId}`);
      }
  ...
  static validateResponses(responses) {
    ...
    if (responseKeys.length !== 44) {
      throw AppError.badRequest('INVALID_RESPONSE_COUNT', 'Invalid response count');
    }
    ...
    for (let i = 1; i <= 44; i++) {
      ...
      if (value === undefined || value === null) {
        throw AppError.badRequest('MISSING_QUESTION', `Missing question ${i}`);
      }
      
      if (!Number.isInteger(value) || value < 1 || value > 5) {
        throw AppError.badRequest('INVALID_VALUE_QUESTION', `Invalid value for question ${i}`);
      }
    }
  }
  ...
  static async submitResponses(userId, responses) {
    ...
    if (!user.hasPersonalityDataConsent()) {
      throw AppError.forbidden('PERSONALITY_CONSENT_REQUIRED', 'Personality data consent is required');
    }
  ...
  static async recalculateProfile(responseId) {
    ...
    if (!response) {
      throw AppError.notFound('BFI44_RESPONSE_NOT_FOUND', 'BFI-44 response not found');
    }
